...
\pnum
An object of dynamic type $\tcode{T}_obj$ is
\defn{type-accessible} through a glvalue of type $\tcode{T}_ref$
if $\tcode{T}_ref$ is similar\iref{conv.qual} to:
\begin{itemize}
\item $\tcode{T}_obj$,

\item a type that is the signed or unsigned type corresponding to $\tcode{T}_obj$, or
...
\pnum
A type $\tcode{T}_call$ is
\defn{call-compatible} with a function type $\tcode{T}_func$
if $\tcode{T}_call$ is the same type as $\tcode{T}_func$ or
if the type ``pointer to $\tcode{T}_func$'' can be
converted to type ``pointer to $\tcode{T}_call$''
